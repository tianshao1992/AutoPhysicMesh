...
# @File    : backend.py
# @Description    : This file defines the unified tensor framework interface required by mpo-engine.
"""

"""This file defines the unified tensor framework interface required by mpo-engine.

The principles of this interface:
* There should be as few interfaces as possible.
* The interface is used by mpo-engine system so it is more important to have
...
def requires_grad(initial_value, dtype=None):
...
def expand_dims(tensor, dim):
    """Expand dim for tensor along given dim.

    Args:
        tensor (Tensor). The input tensor.
        dim (int). dim to expand.
...
def reverse(tensor, dim):
    """Reverse the order of elements along the given dim.

    Args:
        tensor (Tensor). The input tensor.
        dim (int). dim to flip on.

    Returns:
        Tensor: Tensor which is flipped along given dim.
    """


def roll(tensor, shift, dim):
    """Roll the tensor along the given dim (axes).

    Args:
        tensor (Tensor). The input tensor.
        shift (int or tuple of ints). The number of places by which the elements of the
            tensor are shifted.
        dim (int or tuple of ints). dim (axes) along which to roll.
...
def gelu(x):
    """Computes Gaussian Error Linear Unit activation linear."""
...
def max(x, dim=None, keepdim=False, return_index=False):
    ...
    If return_index is True, return the indices of the maximum values as a second tensor.
    """


def min(x, dim=None, keepdim=False, return_index=False):
...
def norm(tensor, ord=None, dim=None, keepdims=False):
...
def ones_like(input_tensor):
    """Create a one-value tensor with the same shape, dtype and context of the given tensor.
...
def einsum(equation, *operands):
    """Implements Einstein summation convention.

    Args:
        equation (str). Subscripts following the Einstein summation convention.
        operands (Tensor). The tensors to be summed over.
