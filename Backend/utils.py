...
# @File    : utils.py
# @Description    : This file is used to check install, or verify backend.
"""
...
def import_taichi():
    """
    Check if taichi is installed.
    """
    try:
        import taichi

        assert taichi  # silence pyflakes
        return True
    except ImportError:
        return False
...
def verify_backend(backend_name):
    ...
    if backend_name not in import_funcs:
        raise NotImplementedError(
            ...
            "Please select backend from taichi, pytorch, jax or paddle."
        )
...
def get_available_backend():
    ...
    """
    backends = ["taichi", "pytorch", "jax", "paddle"]
    import_funcs = {
        "taichi": import_taichi,
        "pytorch": import_pytorch,
        "jax": import_jax,
...
def get_cuda(platform):
    ...
    """
    if platform == "linux":
        cuda_list = [101, 102, 110, 111, 112, 116, 117, 118, 118, 119, 120]
    elif platform == "windows":
        cuda_list = [101, 102, 110, 111, 112, 113, 114, 115, 116, 117, 118, 119, 120]
...
def interactive_install_paddle():
    ...
    sys.exit(
        ...
        "Please manually install a backend, and run MPO again."
    )
