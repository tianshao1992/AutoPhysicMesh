...
def standardize(X_train, X_valid):
    ...
    and then applied to the valid data `X_valid`.
    ...
    Args:
        X_train: A NumPy array of shape (n_samples, n_features). The data used to
            ...
            features axis.
        X_valid: A NumPy array.

    Returns:
        scaler: Instance of ``sklearn.preprocessing.StandardScaler``.
        X_train: Transformed train data.
        X_valid: Transformed valid data.
    """
...
def saveplot(
    ...
    Args:
        ...
        valid_fname (string): Name of the file to save the valid points in.
    ...
    if issave:
        ...
        valid_fname = os.path.join(output_dir, valid_fname)
        save_loss_history(loss_history, loss_fname)
        save_best_state(train_state, train_fname, valid_fname)
...
def plot_loss_history(loss_history, fname=None):
    """Plot the train and valid loss history.
    ...
    loss_valid = np.sum(loss_history.loss_valid, axis=1)

    plt.figure()
    plt.semilogy(loss_history.steps, loss_train, label="Train loss")
    plt.semilogy(loss_history.steps, loss_valid, label="Valid loss")
    for i in range(len(loss_history.metrics_valid[0])):
        plt.semilogy(
            loss_history.steps,
            np.array(loss_history.metrics_valid)[:, i],
            label="Valid metric",
        )
    plt.xlabel("# Steps")
    plt.legend()
...
def save_loss_history(loss_history, fname):
    """Save the training set and valid set loss history to a file."""
    print("Saving loss history to {} ...".format(fname))
    loss = np.hstack(
        (
            np.array(loss_history.steps)[:, None],
            np.array(loss_history.loss_train),
            np.array(loss_history.loss_valid),
            np.array(loss_history.metrics_valid),
        )
    )
    np.savetxt(fname, loss, header="step, loss_train, loss_valid, metrics_valid")
...
def plot_best_state(train_state):
    ...
    y_train, y_valid, best_y, best_ystd = _pack_data(train_state)
    y_dim = best_y.shape[-1]

    # Regression plot
    # 1D
    if train_state.X_valid.shape[-1] == 1:
        idx = np.argsort(train_state.X_valid[:, 0])
        X = train_state.X_valid[idx, 0]
        plt.figure()
        for i in range(y_dim):
            if y_train is not None:
                plt.plot(train_state.X_train[:, 0], y_train[:, i], "ok", label="Train")
            if y_valid is not None:
                plt.plot(X, y_valid[idx, i], "-k", label="True")
        ...
        plt.legend()
    # 2D
    elif train_state.X_valid.shape[-1] == 2:
        for i in range(y_dim):
            plt.figure()
            ax = plt.axes(projection=Axes3D.name)
            ax.plot3D(
                train_state.X_valid[:, 0],
                train_state.X_valid[:, 1],
                best_y[:, i],
                ".",
            )
            ax.set_xlabel("$x_1$")
    ...
    #     plt.figure()
    #     for i in range(y_dim):
    #         plt.plot(train_state.X_valid[:, 0], best_ystd[:, i], "-b")
    #         plt.plot(
    #             train_state.X_train[:, 0],
    #             np.interp(
    #                 train_state.X_train[:, 0], train_state.X_valid[:, 0], best_ystd[:, i]
    #             ),
    #             "ok",
    #         )
    #     plt.xlabel("x")
...
def save_best_state(train_state, fname_train, fname_valid):
    ...
    print("Saving valid data to {} ...".format(fname_valid))
    if y_valid is None:
        valid = np.hstack((train_state.X_valid, best_y))
        if best_ystd is None:
            np.savetxt(fname_valid, valid, header="x, y_pred")
        else:
            valid = np.hstack((valid, best_ystd))
            np.savetxt(fname_valid, valid, header="x, y_pred, y_std")
    else:
        valid = np.hstack((train_state.X_valid, y_valid, best_y))
        if best_ystd is None:
            np.savetxt(fname_valid, valid, header="x, y_true, y_pred")
        else:
            valid = np.hstack((valid, best_ystd))
            np.savetxt(fname_valid, valid, header="x, y_true, y_pred, y_std")
