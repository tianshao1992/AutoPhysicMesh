...
class NavierStokes2DSolver(BasicSolver):
    ...
    def output_transform(self, inn_var, out_var):
        y_hat = inn_var[..., (1,)] * self.L_star  # 原始坐标y
...
class NavierStokes2DEvaluator(BaseEvaluator):
    ...
    def log_plot(self, batch, save_fig='pred_fields'):
        ...
        for time_slice in np.arange(start=0, stop=41, step=5):
            batch_ = self.eval_res(batch)

            real = batch_['all']['target'][time_slice]
            pred = batch_['all']['pred'][time_slice]
            coords = batch_['all']['input'][time_slice]
            fig, axs = plt.subplots(3, 3, num=100+time_slice, figsize=(20, 8))
            self.visual.plot_fields_2D(fig, axs, real, pred, coords,
                                       titles=['真实', '预测', '误差'], field_names=['p', 'u', 'v'],
                                       cmaps=['jet', 'jet', 'coolwarm'])
            if isinstance(save_fig, str):
                fig.savefig(os.path.join(self.visual.save_path, save_fig + "_t_{}.jpg".format(time_slice)))
            self.log_dict.update({'Predicted fields time step at {}'.format(time_slice): wandb.Image(fig2data(fig))})
            plt.close(fig)
